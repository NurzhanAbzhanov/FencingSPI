...
export default function SignInPage({ onSignedIn }: { onSignedIn: (user: PlatformUser) => void }) {
    ...
    async function signIn(event: React.FormEvent) {
        ...
        const profile = await supabase.from("profiles").select("display_name, role").eq("id", data.user.id).single();
        if (profile.error) return setMessage(profile.error.message);
        onSignedIn({ id: data.user.id, name: profile.data.display_name, role: profile.data.role });
    }
