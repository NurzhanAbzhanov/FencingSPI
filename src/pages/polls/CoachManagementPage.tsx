...
export default function CoachManagementPage() {
    ...
    return <PollShell title="Coaches and Administrators" backHref="#/admin">
        ...
        <div className="platform-table-wrap"><table className="platform-table"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Voting</th><th>Status</th><th>Account</th><th /></tr></thead><tbody>{rows.map((row) => <tr key={row.email}><td>{row.displayName}</td><td>{row.email}</td><td>{row.role}</td><td>{row.canVote ? 'Voter' : 'Observer'}</td><td>{row.active ? 'Active' : 'Inactive'}</td><td>{row.linked ? 'Linked' : 'Invitation needed'}</td><td><button className="icon-button" title={`Edit ${row.displayName}`} onClick={() => setForm({ email: row.email, displayName: row.displayName, role: row.role, canVote: row.canVote, active: row.active })}><Save size={16} /></button></td></tr>)}</tbody></table></div>
    </PollShell>;
}
